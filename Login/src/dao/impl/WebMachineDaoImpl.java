...
public class WebMachineDaoImpl extends Basedao implements IWebMachineDao {
	...
	public List<WebMachin> selectWebMachin(String factNo, String yymm,
		...
		List<WebMachin> sList = query.list();
		return sList;
	}
	...
	public WebMachin findById(WebMachinId id) {
		// TODO Auto-generated method stub
		String hql = "from WebMachin w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebMachin machin = (WebMachin) query.uniqueResult();
		return machin;
	}
	...
	public List<WebMachin> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebMachin where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebMachin where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebMachin> list = query.list();
		return list;
	}

	public void delete(WebMachinId id) {
		// TODO Auto-generated method stub
		WebMachin machine = this.findById(id);
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebMachin where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebMachin>list=super.queryForPage(hql.toString(), offset, pageSize, map);
