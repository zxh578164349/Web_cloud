...
import entity.WebPfavgpriceId;

public class WebPfavgPriceDaoImpl extends Basedao implements IWebPfavgPriceDao {
	/**
	 * 瘛餃�撟喳��箄��桀
	 */
	...
	public List<WebPfavgprice> selectPfavgprice(String factNo, String yymm,
		...
		hql = "from WebPfavgprice where 1=1 ";
		...
		List<WebPfavgprice> sList = query.list();
		return sList;
	}
	...
	public WebPfavgprice findById(WebPfavgpriceId id) {
		// TODO Auto-generated method stub
		String hql = "from WebPfavgprice w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebPfavgprice pfavgprice = (WebPfavgprice) query.uniqueResult();
		return pfavgprice;
	}
	...
	public List<WebPfavgprice> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebPfavgprice where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebPfavgprice where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebPfavgprice> list = query.list();
		return list;
	}

	public void delete(WebPfavgpriceId id) {
		// TODO Auto-generated method stub
		WebPfavgprice price = this.findById(id);
		super.delete(price);
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebPfavgprice where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebPfavgprice>list=super.queryForPage(hql.toString(), offset, pageSize, map);
