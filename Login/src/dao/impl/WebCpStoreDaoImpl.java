...
public class WebCpStoreDaoImpl extends Basedao implements IWebCpStroreDao {
	...
	public List<WebCpstore> selectCpStore(String factNo, String yymm, int page,
		...
		List<WebCpstore> sList = query.list();
		return sList;
	}
	...
	public WebCpstore findById(WebCpstoreId id) {
		// TODO Auto-generated method stub
		String hql = "from WebCpstore w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebCpstore cpstore = (WebCpstore) query.uniqueResult();
		return cpstore;
	}
	...
	public List<WebCpstore> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebCpstore where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebCpstore where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebCpstore> list = query.list();
		return list;
	}

	public void delete(WebCpstoreId id) {
		// TODO Auto-generated method stub
		WebCpstore store = this.findById(id);
		super.delete(store);
	}
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebCpstore where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebCpstore>list=super.queryForPage(hql.toString(), offset, pageSize, map);
