...
import entity.WebZwcashId;

public class WebZwcashDaoImpl extends Basedao implements IWebZwcashDao {
	/**
	 * 瘛餃�瘞湧瘝�
	 */
	...
	public List<WebZwcash> selectZwcash(String factNo, String yymm, int page,
		...
		List<WebZwcash> sList = query.list();
		return sList;
	}
	...
	public WebZwcash findById(WebZwcashId id) {
		// TODO Auto-generated method stub
		String hql = "from WebZwcash w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebZwcash zwcash = (WebZwcash) query.uniqueResult();
		return zwcash;
	}
	...
	public List<WebZwcash> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebZwcash where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebZwcash where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebZwcash> list = query.list();
		return list;
	}

	public void delete(WebZwcashId id) {
		// TODO Auto-generated method stub
		WebZwcash zwcash = this.findById(id);
		super.delete(zwcash);
	}
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebZwcash where 1=1");
		int allrow=0;
		...
		List<WebZwcash>list=super.queryForPage(hql.toString(), offset, pageSize, map);
