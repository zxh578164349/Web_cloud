...
public class WebProdutedDaoImpl extends Basedao implements IWebProdutedDao {
	...
	public List<Webproduted> selectProduted(String factNo, String yymm,
		...
		hql = "from Webproduted where 1=1 ";
		...
		if (size == 0) {
			List<Webproduted> sList = query.list();
			size = sList.size();
		}
		if (page == 0 && rows == 0) {
	...
	public Webproduted findById(WebprodutedId id) {
		String hql = "from Webproduted w where w.id.factNo=? and w.id.factCode=? and w.id.yymm=? ";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getFactCode());
		query.setDate(2, id.getYymm());
		Webproduted produted = (Webproduted) query.uniqueResult();
		return produted;
	}
	...
	public List<Webproduted> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from Webproduted where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from Webproduted where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<Webproduted> list = query.list();
		return list;
	...
	public Webproduted selBycan(String factNo, Date date, String factCode) {
		String hql = "from Webproduted where id.factNo=? and id.factCode=? and id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, factNo);
		query.setString(1, factCode);
		query.setDate(2, date);
		Webproduted mix2 = (Webproduted) query.uniqueResult();
		return mix2;
	}
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		List<Webproduted>list=super.queryForPage(hql.toString(), offset, pageSize, map);
	...
	public List<Webproduted> findByAny(String factNo, String beginDate,
		...
		hql.append(" order by id.factNo,id.factCode,id.yymm desc");
		List<Webproduted>list=super.getAllWithNoPage(hql.toString(), map);
		return list;
	}

	public Webproduted findById(String factNo, String factCode, String yymm) {
		// TODO Auto-generated method stub
		String hql="from Webproduted where id.factNo=? and id.factCode=? and to_char(id.yymm,'yyyymm')=?";
		Query query=getSession().createQuery(hql);
		query.setString(0, factNo);
		query.setString(1,factCode);
