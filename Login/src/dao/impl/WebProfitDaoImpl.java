...
import entity.WebProfitId;

public class WebProfitDaoImpl extends Basedao implements IWebProfitDao {
	/**
	 * 瘛餃���銵�
	 */
	...
	public List<WebProfit> selectProfit(String factNo, String yymm, int page,
		...
		List<WebProfit> sList = query.list();
		return sList;
	...
	public WebProfit findById(WebProfitId id) {
		// TODO Auto-generated method stub
		String hql = "from WebProfit w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebProfit profit = (WebProfit) query.uniqueResult();
		return profit;
	}
	...
	public List<WebProfit> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebProfit where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebProfit where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebProfit> list = query.list();
		return list;
	}

	public void delete(WebProfitId id) {
		// TODO Auto-generated method stub
		WebProfit profit = this.findById(id);
		super.delete(profit);
	}
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebProfit where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebProfit>list=super.queryForPage(hql.toString(), offset, pageSize, map);
