...
import entity.WebOutputinvId;

public class WebOutputInvImpl extends Basedao implements IWebOutputInvDao {
	/**
	 * 
	 */
	...
	public List<WebOutputinv> selectOutputInv(String factNo, String yymm,
		...
		hql = "from WebOutputinv where 1=1 ";
		...
		List<WebOutputinv> sList = query.list();
	...
	public WebOutputinv findById(WebOutputinvId id) {
		// TODO Auto-generated method stub
		String hql = "from WebOutputinv w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=? and w.id.factCode=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		query.setString(3, id.getFactCode());
		WebOutputinv outputinv = (WebOutputinv) query.uniqueResult();
		return outputinv;
	}
	...
	public List<WebOutputinv> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebOutputinv where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebOutputinv where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebOutputinv> list = query.list();
		return list;
	}

	public void delete(WebOutputinvId id) {
		// TODO Auto-generated method stub
		WebOutputinv inv = this.findById(id);
		super.delete(inv);
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebOutputinv where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebOutputinv>list=super.queryForPage(hql.toString(), offset, pageSize, map);
