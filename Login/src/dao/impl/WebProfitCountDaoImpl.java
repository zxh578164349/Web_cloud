...
import entity.WebProfitcountId;

public class WebProfitCountDaoImpl extends Basedao implements
		IWebProfitCountDao {
	/**
	 * 瘛餃����舐蜇
	 */
	...
	public List<WebProfitcount> selectProfitcount(String factNo, String yymm,
		...
		hql = "from WebProfitcount where 1=1 ";
		...
		List<WebProfitcount> sList = query.list();
		return sList;
	}
	...
	public WebProfitcount findById(WebProfitcountId id) {
		// TODO Auto-generated method stub
		String hql = "from WebProfitcount w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebProfitcount profitcount = (WebProfitcount) query.uniqueResult();
		return profitcount;
	}
	...
	public List<WebProfitcount> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebProfitcount where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebProfitcount where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebProfitcount> list = query.list();
		return list;
	}

	public void delete(WebProfitcountId id) {
		// TODO Auto-generated method stub
		WebProfitcount count = this.findById(id);
		super.delete(count);
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebProfitcount where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebProfitcount>list=super.queryForPage(hql.toString(), offset, pageSize, map);
