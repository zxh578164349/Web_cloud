...
import entity.WebMatsId;

public class WebMatsDaoImpl extends Basedao implements IWebMatsDao {
	/**
	 * 瘛餃��脫��亙��
	 */
	...
	public WebMats findById(WebMatsId id) {
		// TODO Auto-generated method stub
		String hql = "from WebMats w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebMats mats = (WebMats) query.uniqueResult();
		return mats;
	}
	...
	public List<WebMats> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebMats where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebMats where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebMats> list = query.list();
		return list;
	}

	public void delete(WebMatsId id) {
		// TODO Auto-generated method stub
		WebMats mat = this.findById(id);
		super.delete(mat);
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebMats where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebMats>list=super.queryForPage(hql.toString() , offset, pageSize, map);
