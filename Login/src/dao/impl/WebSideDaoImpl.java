...
import entity.WebSideId;
import entity.WebZwcash;

public class WebSideDaoImpl extends Basedao implements IWebSideDao {
	/**
	 * 瘛餃���銝����
	 */
	...
	public WebSide findById(WebSideId id) {
		// TODO Auto-generated method stub
		String hql = "from WebSide w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebSide side = (WebSide) query.uniqueResult();
		return side;
	}
	...
	public List<WebSide> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebSide where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebSide where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebSide> list = query.list();
		return list;
	}

	public void delete(WebSideId id) {
		// TODO Auto-generated method stub
		WebSide side = this.findById(id);
		super.delete(side);
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebSide where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebSide>list=super.queryForPage(hql.toString(), offset, pageSize, map);
