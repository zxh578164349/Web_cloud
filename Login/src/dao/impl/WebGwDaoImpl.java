...
public class WebGwDaoImpl extends Basedao implements IWebGwDao {
	...
	public WebGw findById(WebGwId id) {
		// TODO Auto-generated method stub
		String hql = "from WebGw w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebGw gw = (WebGw) query.uniqueResult();
		return gw;
	}
	...
	public List<WebGw> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebGw where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebGw where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebGw> list = query.list();
		return list;
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebGw where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebGw>list=super.queryForPage(hql.toString(), offset, pageSize, map);
