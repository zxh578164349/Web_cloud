...
import entity.WebMatstoreId;

public class WebMatstoreDaoImpl extends Basedao implements IWebMatstoreDao {
	/**
	 * 瘛餃��典���摨怠�
	 */
	public void addMatstore(WebMatstore matstore) {
		// TODO Auto-generated method stub
		super.merge(matstore);
	}
	...
	public List<WebMatstore> selectMatstore(String factNo, String yymm,
		...
		hql = "from WebMatstore where 1=1 ";
		...
		List<WebMatstore> sList = query.list();
	...
	public WebMatstore findById(WebMatstoreId id) {
		// TODO Auto-generated method stub
		String hql = "from WebMatstore w where w.id.factNo=? and w.id.billNo=? and w.id.yymm=?";
		Query query = getSession().createQuery(hql);
		query.setString(0, id.getFactNo());
		query.setString(1, id.getBillNo());
		query.setDate(2, id.getYymm());
		WebMatstore matstore = (WebMatstore) query.uniqueResult();
		return matstore;
	}
	...
	public List<WebMatstore> selByYymm(String yymm, String factNo) {
		...
		if (factNo != null) {
			if (!factNo.equals("tw")) {
				String hql = "from WebMatstore where id.yymm =:yymm and id.factNo=:factNo";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
				query.setParameter("factNo", factNo);
			} else {
				String hql = "from WebMatstore where id.yymm =:yymm";
				query = getSession().createQuery(hql);
				query.setParameter("yymm", s);
			}
		}
		List<WebMatstore> list = query.list();
		return list;
	}

	public void delete(WebMatstoreId id) {
		// TODO Auto-generated method stub
		WebMatstore store = this.findById(id);
		super.delete(store);
	...
	public PageBean findPageBean(int pageSize, int page, String factNo,
		...
		hql.append("from WebMatstore where 1=1");
		if(factNo!=null&&!factNo.equals("")&&!factNo.equals("tw")&&!factNo.equals("nothing")){
		...
		List<WebMatstore>list=super.queryForPage(hql.toString(), offset, pageSize, map);
