...
public class AutoSendWebobjC extends QuartzJobBean {
	...
	public void init(){
		...
		try {
			...
			if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY){	
				...
				HttpMethod method = new GetMethod(pc.getpUrl()+"/webobjc_print_tw2_c?yymmdd="+yymmdd+"&emailMk=1");
				//HttpMethod method = new GetMethod(pc.getPurllocal()+"/webobjc_print_tw2_c?yymmdd="+yymmdd+"&emailMk=1");				
				...
				//List<WebEmailAll> email = eSer.findEmail(4, "0");//只發送我自己的郵箱
				...
				//List<WebEmailAll> Cc = eSer.findEmail(4, "1");//只抄送我自己的郵箱
				...
				//郵件內容
				String affixName=yymmdd+"工廠日報.xlsx";
	...
	public String print_manual() throws ParseException {
		...
		try {
			...
			if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) {
				...
				HttpClient client=new HttpClient();
				HttpMethod method = new GetMethod(pc.getpUrl()+"/webobjc_print_tw2?yymmdd="+yymmdd+"&emailMk=1");					
				//HttpMethod method = new GetMethod(pc.getPurllocal()+"/webobjc_print_tw2_c?yymmdd="+yymmdd+"&emailMk=1");
				//HttpMethod method = new GetMethod("http://172.17.18.173:8080/WebLogin/webobjc_print_tw2?yymmdd="+yymmdd+"&emailMk=1");
				//HttpMethod method=new GetMethod("http://203.85.73.161/"+pname+"/print2Ypoi_print2Y_hb?sdate=" + yymm + "&edate=" + yymm+ "&emailMk=1&type=Excel2003");// (在不同的機器上注意修改IP和端口)						
				//HttpMethod method=new GetMethod("http://172.17.18.173:8080/"+pname+"/print2Ypoi_print2Y_hb?sdate="+yymm+"&edate="+yymm+"&emailMk=1&type=Excel2003");
				// HttpMethod method=new GetMethod("http://localhost:8080/"+pname+"/print2Ypoi_print2Y_hb?sdate="+yymm+"&edate="+yymm+"&emailMk=1&type=Excel2003");
				client.executeMethod(method);
				...
				AutoSendEmailAction send=new AutoSendEmailAction();
				String affixName=yymmdd+"工廠日報.xls";				
				// 郵件內容				
				StringBuffer content=new StringBuffer();
				content.append(this.findNoInput(ac, yymmdd));
				content.append("本郵件自動發送,請勿回復!如需回复，請回复到"+pc.getpEmail()+"咨訊室或者"+pc.getPlgx()+"譚香林!");					
				// 發送郵件
				send.sendmail(mail,cc,yymmdd+"工廠日報",content.toString(),affixName,filepath);
	...
	public String findNoInput(ApplicationContext ac,String yymmdd){
		...
		IWebObjsCServices webobjcservices=(IWebObjsCServices)ac.getBean("webobjcservices");
		List<String[]>list=webobjcservices.findNoInput(yymmdd);
