...
public class WebcashoutAction extends ActionSupport implements ServletResponseAware{
	...
	public void report() throws IOException, ParseException{
		...
		list_col_top.add("�t�O���A");
		list_col_top.add("�w�p�Ͳ����(��)");
		list_col_top.add("�w�p�д����(��)");
		list_col_top.add("�w�p�дڪ��B(USD)");
		//(2)�ƾ�
		/*yymm="201507";
		factNo="all";*/
		...
		String title="";
		if(factNo.equals("all")){			
			title="�Ҧ��u�t"+yymm+"�Ͳ��P�дڹF�����p�έp���R��";
		}else{
			title="("+list_fact.get(0).getFactSname()+")"+yymm+"�Ͳ��P�дڹF�����p�έp���R��";
		}		
		/**(�U��)�ƾڷ�**/
		//(1)���(yyyy/MM)�榡
		...
		for(int i=0;i<list_days.size();i++){
			list_col_buttom.add("����ڼ�");
			list_col_buttom.add("�ֿn��");
			list_col_buttom.add("�ֿn�F��%");
			list_col_buttom.add("�ֿn���");
		}		
		//(3)���Y
		...
		for(int i=0;i<list_fact.size();i++){
			list_col_buttom2.add("�Ͳ��ˮ�");
			list_col_buttom2.add("������");
			list_col_buttom2.add("�����B");
			list_col_buttom2.add("�д����");
			list_col_buttom2.add("�дڪ��B");
		...
		//(4)�ƾ�
		List<String>list_days2=this.getDates(yymm).get(1);//���(yyyyMMdd)�榡
		...
		//���~�h�`���n�ϹL�ӡA�~�h�`���ѼơA���`���t�O
			int a_length=list_vcashout_all.get(0).size();//�~�h�`������,�]���U�Ӥ�������׳��@�ˡA�G��Ĥ@��
			List<Double[]>list_total_top=new ArrayList<Double[]>();
			for(int i=0;i<a_length;i++){//�~�`���Ѽ�
				...
				if(list_vcashout_all.get(0).get(i)!=null){
					yymmdd=new SimpleDateFormat("yyyyMMdd").format(list_vcashout_all.get(0).get(i).getId().getYymmdd());//��X�U��yymmdd
				}				
				for(int j=0;j<list_fact.size();j++){//���`���t�O
		...
		for(int x=0;x<list_top_all.size();x++){//start for1
			...
			/*************y�b���=2,x=2****************/						
			for(int y=0;y<obj.length-1;y++){//start for2  �`�N,obj.length-1,�]��obj���s�W�F�@��type
				...
				if(temp==null){
					//temp=0.0;
					cs_temp=cs_red;
					sheet.getRow(index_y+1+y).getCell(2+5*x).setCellValue("�L�ƾ�");
				}else{
					cs_temp=cs;
		...
		sheet.getRow(index_y).getCell(0).setCellValue("���");
		...
		sheet.getRow(index_y).getCell(1).setCellValue("����");
		...
		for(int i=0;i<list_vcashout_all.size();i++){//start for1
			...
			for(int j=0;j<list_vcashout.size();j++){//start for2
				...
				if(vcashout!=null){
					...
					cs_temp=cs_red;
					sheet.getRow(index_y+4*j).getCell(2+5*i).setCellValue("�L�ƾ�");
					sheet.getRow(index_y+4*j).getCell(3+5*i).setCellValue("�L�ƾ�");
					sheet.getRow(index_y+4*j).getCell(4+5*i).setCellValue("�L�ƾ�");
					sheet.getRow(index_y+4*j).getCell(5+5*i).setCellValue("�L�ƾ�");
					sheet.getRow(index_y+4*j).getCell(6+5*i).setCellValue("�L�ƾ�");				
					...
					//sheet.getRow(8+4*j).getCell(6+5*i).setCellValue("�L�ƾ�");
				}				
		...
		int msie=ServletActionContext.getRequest().getHeader("USER-AGENT").toLowerCase().indexOf("msie");//�P�_�s������,�j��0�h��IE�s��
		if(msie>0){
			fileName=java.net.URLEncoder.encode(fileName,"utf-8");//�ѨMIE�����󤣯�U����D
		}else{
			fileName=new String(fileName.getBytes("utf-8"),"iso8859-1");//�ѨM�DIE����W�ýX���D
		}
