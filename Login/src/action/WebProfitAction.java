...
public class WebProfitAction extends ActionSupport {
	private WebProfit profit;
	...
	public WebProfit getProfit() {
		return profit;
	}

	public void setProfit(WebProfit profit) {
		this.profit = profit;
	}
	...
	private List<WebProfit> listmat;
	...
	public List<WebProfit> getListmat() {
		return listmat;
	}

	public void setListmat(List<WebProfit> listmat) {
	 ...
	 */
	public String addProfit() throws ParseException {
		...
		ym.append(year);
		ym.append(month);
		if (profit.getId().getBillNo() == null) {
			StringBuffer index = new StringBuffer();
			String typeId = ActionContext.getContext().getSession()
					.get("typeid").toString();
			index.append(typeId);
			// index.append("QQ");
			String factno = profit.getId().getFactNo();
			index.append(factno);
			...
			profit.getId().setBillNo(temp);

			d = format.parse(ym.toString());
			profit.getId().setYymm(d);
			profit.setDateTime(date);
			profitSer.addProfit(profit);
		} else {
			d = format.parse(yymm);
			profit.getId().setYymm(d);
			profit.setDateTime(date);
	...
	public void list() {
		...
		List<WebProfit> bList = profitSer.selProfit(factNos, yymm,
				pages.getPage(), pages.getRows());
		if (bList.size() == 0) {
			pages.setPage(0);
		}
		ActionContext.getContext().getSession().put("profitList", bList);
		pages.setCount(profitSer.totlePage(factNos, yymm));
			...
			for (int i = 0; i < files.length; i++) {
				...
				for (int b = 0; b < infoList.size(); b++) {
					...
					if (b < 1) {
						...
						WebProfit profit = new WebProfit();
						WebProfitId id = new WebProfitId();
						for (int g = 0; g < names.size(); g++) {
							...
							if (mes[g] != null || !mes[g].equals("")) {
								if (name.trim().equals("年月")) {
									...
									profit.setFactCode(mes[g]);
								} else if (name.trim().equals("銷售收入")) {
									profit.setInvCount(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("成本合計")) {
									profit.setCostCount(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("工資合計")) {
									profit.setWageCount(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("費用合計")) {
									profit.setCashCount(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("其他")) {
									profit.setOtherCount(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("本期損益")) {
									profit.setThisProfit(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("日期起")) {
									profit.setDateB(mes[g]);
								} else if (name.trim().equals("日期止")) {
									profit.setDateE(mes[g]);
								} else if (name.trim().equals("鎖定")) {
									profit.setLockMk(mes[g]);
								} else if (name.trim().equals("輸入者工號")) {
									profit.setUserNo(mes[g]);
								} else if (name.trim().equals("輸入時間")) {
									profit.setDateTime(mes[g]);
	...
	public String getList(){
		...
		bean=profitSer.findPageBean(25, page, factNo, yymm);
		return "showList";
	}
	public String getList2(){
		ActionContext.getContext().getApplication().clear();
		if(factNo!=null&&!factNo.equals("")){
			ActionContext.getContext().getApplication().put("webprofit-factno", factNo);
		}
		if(yymm!=null&&!yymm.equals("")){
			ActionContext.getContext().getApplication().put("webprofit-yymm", yymm);
		}
		bean=profitSer.findPageBean(25, page, factNo, yymm);
		return "showList1";
	}
	public String getList3(){
		factNo=(String)ActionContext.getContext().getApplication().get("webprofit-factno");
		yymm=(String)ActionContext.getContext().getApplication().get("webprofit-yymm");
		if(factNo==null||factNo.equals("")){
			factNo=(String)ActionContext.getContext().getSession().get("factNo");
		}
		bean=profitSer.findPageBean(25, page, factNo, yymm);
