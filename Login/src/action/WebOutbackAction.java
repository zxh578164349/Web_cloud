...
public class WebOutbackAction extends ActionSupport {
	...
	private List<WebOutback> listmat;
	...
	public List<WebOutback> getListmat() {
		return listmat;
	}

	public void setListmat(List<WebOutback> listmat) {
	 ...
	 */
	public String addBack() throws ParseException {
	...
	public void list() {
		...
		List<WebOutback> bList = outSer.selOutback(factNos, yymm,
				pages.getPage(), pages.getRows());

		if (bList.size() == 0) {
			pages.setPage(0);
		}
		ActionContext.getContext().getSession().put("outbackList", bList);
		pages.setCount(outSer.totlePage(factNos, yymm));
	...
	public String findById() {
		...
		ActionContext.getContext().put("factAreas_other", list);
		back = outSer.findById(id);
		return "findById";
	}
			...
			for (int i = 0; i < files.length; i++) {
				...
				for (int b = 0; b < infoList.size(); b++) {
					...
					if (b < 1) {
						...
						WebOutback outback = new WebOutback();
						WebOutbackId id = new WebOutbackId();
						for (int g = 0; g < names.size(); g++) {
							...
							if (mes[g] != null || !mes[g].equals("")) {
								if (name.trim().equals("年月")) {
									...
									outback.setFactCode(mes[g]);
								} else if (name.trim().equals("出貨數")) {
									outback.setOutCount(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("退貨數")) {
									outback.setBackCount(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("退貨率")) {
									outback.setBackExp(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("日期起")) {
									outback.setDateB(mes[g]);
								} else if (name.trim().equals("日期止")) {
									outback.setDateE(mes[g]);
								} else if (name.trim().equals("鎖定")) {
									outback.setLockMk(mes[g]);
								} else if (name.trim().equals("輸入者工號")) {
									outback.setUserNo(mes[g]);
								} else if (name.trim().equals("輸入時間")) {
									outback.setDateTime(mes[g]);
	...
	public String getList(){
		...
		bean=outSer.findPageBean(25, page, factNo, yymm);
		return "showList";
	}
	public String getList2(){
		ActionContext.getContext().getApplication().clear();
		if(factNo!=null&&!factNo.equals("")){
			ActionContext.getContext().getApplication().put("weboutback-factno", factNo);
		}
		if(yymm!=null&&!yymm.equals("")){
			ActionContext.getContext().getApplication().put("weboutback-yymm", yymm);
		}
		bean=outSer.findPageBean(25, page, factNo, yymm);
		return "showList1";
	}
	public String getList3(){
		factNo=(String)ActionContext.getContext().getApplication().get("weboutback-factno");
		yymm=(String)ActionContext.getContext().getApplication().get("weboutback-yymm");
		if(factNo==null||factNo.equals("")){
			factNo=(String)ActionContext.getContext().getSession().get("factNo");
		}
		bean=outSer.findPageBean(25, page, factNo, yymm);
