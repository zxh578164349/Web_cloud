...
public class WebPfavgPriceAction extends ActionSupport {
	private WebPfavgprice price;
	...
	public WebPfavgprice getPrice() {
		return price;
	}

	public void setPrice(WebPfavgprice price) {
		this.price = price;
	}
	...
	private List<WebPfavgprice> listmat;

	public List<WebPfavgprice> getListmat() {
		return listmat;
	}

	public void setListmat(List<WebPfavgprice> listmat) {
	 ...
	 */
	public String addprice() throws ParseException {
		...
		ym.append(year);
		ym.append(month);
		if (price.getId().getBillNo() == null) {
			StringBuffer index = new StringBuffer();
			String typeId = ActionContext.getContext().getSession()
					.get("typeid").toString();
			index.append(typeId);
			// index.append("QQ");
			String factno = price.getId().getFactNo();
			index.append(factno);
			index.append(date);
			...
			price.getId().setBillNo(temp);

			d = format.parse(ym.toString());
			price.getId().setYymm(d);
			price.setDateTime(date);
			favgSer.addprice(price);
	...
	public void list() {
		...
		List<WebPfavgprice> bList = favgSer.selPfavgprice(factNos, yymm,
				pages.getPage(), pages.getRows());
		if (bList.size() == 0) {
			pages.setPage(0);
		}
		ActionContext.getContext().getSession().put("pfavgpriceList", bList);
		pages.setCount(favgSer.totlePage(factNos, yymm));
			...
			for (int i = 0; i < files.length; i++) {
				...
				for (int b = 0; b < infoList.size(); b++) {
					...
					if (b < 1) {
						...
						WebPfavgprice pfavgprice = new WebPfavgprice();
						WebPfavgpriceId id = new WebPfavgpriceId();
						for (int g = 0; g < names.size(); g++) {
							...
							if (mes[g] != null || !mes[g].equals("")) {
								if (name.trim().equals("年月")) {
									...
									id.setYymm(s1);
									pfavgprice.setId(id);
								} else if (name.trim().equals("單價")) {
									pfavgprice.setPrice(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("日期起")) {
									pfavgprice.setDateB(mes[g]);
								} else if (name.trim().equals("廠別狀態")) {
									pfavgprice.setFactCode(mes[g]);
								} else if (name.trim().equals("日期止")) {
									pfavgprice.setDateE(mes[g]);
								} else if (name.trim().equals("鎖定")) {
									pfavgprice.setLockMk(mes[g]);
								} else if (name.trim().equals("輸入者工號")) {
									pfavgprice.setUserNo(mes[g]);
								} else if (name.trim().equals("輸入時間")) {
									pfavgprice.setDateTime(mes[g]);
	...
	public String getList(){
		...
		bean=favgSer.findPageBean(25, page, factNo, yymm);
		return "showList";
	}
	public String getList2(){
		ActionContext.getContext().getApplication().clear();
		if(factNo!=null&&!factNo.equals("")){
			ActionContext.getContext().getApplication().put("favg-factno", factNo);
		}
		if(yymm!=null&&!yymm.equals("")){
			ActionContext.getContext().getApplication().put("favg-yymm", yymm);
		}
		bean=favgSer.findPageBean(25, page, factNo, yymm);
		return "showList1";
	}
	public String getList3(){
		factNo=(String)ActionContext.getContext().getApplication().get("favg-factno");
		yymm=(String)ActionContext.getContext().getApplication().get("favg-yymm");
		if(factNo==null||factNo.equals("")){
			factNo=(String)ActionContext.getContext().getSession().get("factNo");
		}
		bean=favgSer.findPageBean(25, page, factNo, yymm);
