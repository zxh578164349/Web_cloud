...
public class WebMatsAction extends ActionSupport {
	private WebMats mats;
	...
	public WebMats getMats() {
		return mats;
	}

	public void setMats(WebMats mats) {
		this.mats = mats;
	}
	...
	public List<WebMats> getListmat() {
		return listmat;
	}

	public void setListmat(List<WebMats> listmat) {
	 ...
	 */
	public String addMats() throws ParseException {
		...
		ym.append(year);
		ym.append(month);
		if (mats.getId().getBillNo() == null) {
			StringBuffer index = new StringBuffer();
			String typeId = ActionContext.getContext().getSession()
					.get("typeid").toString();
			index.append(typeId);
			// index.append("QQ");
			String factno = mats.getId().getFactNo();
			index.append(factno);
			index.append(date);
			...
			mats.getId().setBillNo(temp);

			d = format.parse(ym.toString());
			mats.getId().setYymm(d);
			mats.setDateTime(date);
			matSer.addMats(mats);
	...
	public void list() {
		...
		List<WebMats> bList = matSer.selMats(factNos, yymm, pages.getPage(),
				pages.getRows());
		if (bList.size() == 0) {
			pages.setPage(0);
		}
		ActionContext.getContext().getSession().put("matsList", bList);
		pages.setCount(matSer.totlePage(factNos, yymm));
			...
			for (int i = 0; i < files.length; i++) {
				...
				for (int b = 0; b < infoList.size(); b++) {
					...
					if (b < 1) {
						...
						WebMats mats = new WebMats();
						WebMatsId id = new WebMatsId();
						for (int g = 0; g < names.size(); g++) {
							...
							if (mes[g] != null || !mes[g].equals("")) {
								if (name.trim().equals("年月")) {
									...
									mats.setFactCode(mes[g]);
								} else if (name.trim().equals("色料")) {
									mats.setMatC(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("藥品")) {
									mats.setMatS(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("數量單耗")) {
									mats.setCountDanhao(Double.valueOf(mes[g])
											.doubleValue());
								} else if (name.trim().equals("金額單耗")) {
									mats.setMoneyDanhao(Double.valueOf(mes[g])
											.doubleValue());
									...
									mats.setLockMk(mes[g]);
								} else if (name.trim().equals("輸入者工號")) {
									mats.setUserNo(mes[g]);
								} else if (name.trim().equals("輸入時間")) {
									mats.setDateTime(mes[g]);
	...
	public String getList(){
		...
		bean=matSer.findPageBean(25, page, factNo, yymm);
		return "showList";
	}
	public String getList2(){
		ActionContext.getContext().getApplication().clear();
		if(factNo!=null&&!factNo.equals("")){
			ActionContext.getContext().getApplication().put("webmat-factno", factNo);
		}
		if(yymm!=null&&!yymm.equals("")){
			ActionContext.getContext().getApplication().put("webmat-yymm", yymm);
		}
		bean=matSer.findPageBean(25, page, factNo, yymm);
		return "showList1";
	}
	public String getList3(){
		factNo=(String)ActionContext.getContext().getApplication().get("webmat-factno");
		yymm=(String)ActionContext.getContext().getApplication().get("webmat-yymm");
		if(factNo==null||factNo.equals("")){
			factNo=(String)ActionContext.getContext().getSession().get("factNo");
		}
		bean=matSer.findPageBean(25, page, factNo, yymm);
