...
<script type="text/javascript">
	$(function() {
		...
		demo.tipmsg.w["*0-4"] = "只能數字且不超過4位數,可保留三位以內小數";
	});
...
<body>
	<form action="avprice_addprice" method="post" id="form">
		<table width="100%" align="center" cellspacing="0" cellpadding="0"
			...
			<tbody id="tb_list_info2">
				<s:if test="price==null">
					<tr>
						...
						<td class="td_input"><s:if test="#session.factNo!='tw'">
								<input type="text" style="color:blue" name="price.id.factNo"
									value="${factNo}" readonly />
							</s:if> <s:if test="#session.factNo=='tw'">
								<select style="color:blue" name="price.id.factNo"
					...
					<tr>
						...
						<td class="td_input">年 <span><select name="year"
							...
							</select> </span> <!-- <input type="text" name="price.id.yymm"   onClick="WdatePicker()" id="yymm"  onblur="clear1()" value="系統當前日期" disabled/> -->
							<span id="syymm"></span></td>
						<td class="td_show_title">廠別狀態</td>
						<td class="td_input"><s:if test="#session.factNo=='tw'">
								<select name="price.factCode" id="dwrFactArea" datatype="*">
									<option value="">請選擇廠別狀態</option>
								</select>
							</s:if> <s:else>
								<select name="price.factCode" datatype="*">
									<option value="">請選擇廠別狀態</option>
				...
				<s:if test="price!=null">
					<tr>
						...
						<td class="td_input"><font color="blue"><input
								...
								value="<s:property value='price.id.factNo'/>"
								name="price.id.factNo" readonly /> </font>
						</td>
						<td class="td_show_title">單號</td>
						<td class="td_input"><font color="blue"><input
								type="text" id="billNo" style="color:blue"
								value="<s:property value='price.id.billNo'/>"
								name="price.id.billNo" readonly /> </font>
						</td>
					</tr>
					<tr>
						<td class="td_show_title">年月</td>
						<td class="td_input"><input type="text" style="color:blue"
							id="yymm" value="<s:date name='price.id.yymm' format='yyyyMM'/>"
							name="yymm" readonly /></td>
						<td class="td_show_title">廠別狀態</td>
						<td class="td_input">
							<%-- <input type="text" name="price.factCode" value="<s:property value='price.factCode' />"> --%>
							<select name="price.factCode" datatype="*">
								<option value="">請選擇廠別狀態</option>
								<s:iterator value="#session.factAreas_other" id="temp">
									<s:if test="#attr.temp==price.factCode">
				...
				<tr>
					<td class="td_show_title">單價</td>
					<td class="td_input"><input type="text" name="price.price"
						value="<s:property value='price.price'/>" datatype="*0-4">
					</td>
					<td class="td_show_title">日期起</td>
					<td class="td_input"><input type="text" name="price.dateB"
						value="<s:property value='price.dateB'/>" onClick="WdatePicker()"
						readonly class="Wdate"></td>

				</tr>
				<tr>
					<td class="td_show_title">日期止</td>
					<td class="td_input"><input type="text" name="price.dateE"
						id="year" onClick="WdatePicker()"
						value="<s:property value='price.dateE'/>" readonly class="Wdate" />
					</td>
					<td class="td_show_title">鎖定</td>
					<td class="td_input"><input type="text" name="price.lockMk"
						value="<s:property value='price.lockMk'/>">
					</td>
				</tr>
				<tr>
					<td class="td_show_title">輸入者工號</td>
					<td class="td_input"><input type="text" name="price.userNo"
						value="${loginUser.workno}" style="color:blue" readonly>
					</td>
					<td class="td_show_title">輸入時間</td>
					<td class="td_input"><input type="text" name="price.dateTime"
