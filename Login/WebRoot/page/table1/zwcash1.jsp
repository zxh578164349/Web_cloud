...
<title>My JSP 'zwcash1.jsp' starting page</title>
	...
	<table id="tb" >
		...
		<s:iterator value="bean.list" status="x" id="temp">
			<tr onmousemove="click_color(this)" onmouseout="move(this)">
				...
				<td><s:property value="%{formatDouble(realDemo)}" /></td>
				<td><s:property value="%{formatDouble(lightDu)}" /></td>
				<td><s:property value="%{formatDouble(lightDemo)}"/></td>
				<td><s:property value="%{formatDouble(lightUsd)}" /></td>
				<td><s:property value="%{formatDouble(lightExp)}" /></td>
				<td><s:property value="%{formatDouble(gasT%)}" /></td>
				<td><s:property value="%{formatDouble(gasDemo)}" /></td>
				<td><s:property value="%{formatDouble(gasUsd)}" /></td>
				<td><s:property value="%{formatDouble(gasExp)}" /></td>
				<td><s:property value="%{formatDouble(oilLi)}" /></td>
				<td><s:property value="%{formatDouble(oilDemo)}" /></td>
				<td><s:property value="%{formatDouble(oilUsd)}" /></td>
				<td><s:property value="%{formatDouble(oilExp)}" /></td>

				<td><s:property value="%{formatDouble(waterDu)}" /></td>
				<td><s:property value="%{formatDouble(waterDemo)}" /></td>
				<td><s:property value="%{formatDouble(waterUsd)}" /></td>
				<td><s:property value="%{formatDouble(waterExp)}" /></td>
				...
				<s:if test='#session.loginUser.userread!="1"'>
				<td>
					<form action="cash_findById" method="post" id="subform${x.index}">
					...
					&nbsp;
					<form action="cash_delete2" method="post" id="subform2${x.index}"
						style="float:left">
						<input type="hidden" value="<s:property value='id.factNo'/>"
							name="id.factNo" /> <input type="hidden"
							value="<s:property value='id.billNo'/>" name="id.billNo" /> <input
